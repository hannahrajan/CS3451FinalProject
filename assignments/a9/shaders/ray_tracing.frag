...
void initScene() 
{
    ...
    //camera = Camera(vec3(0, 15, 50), vec3(5, 0, 0), vec3(0, 3 - 0.75, -3 - 0.75), vec3(-2.5, -1.5, -1 + 0.35));
    ...
    materials[1].kd = vec3(0.8, 0.2, 0.8);
    ...
    materials[2].kd = vec3(0.2, 0.8, 0.3);
    materials[2].ks = vec3(0.8);
    materials[2].shininess = 250;
...
vec3 shadingPhong(Light light, int matId, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 amb = vec3(ka * light.Ia);
    vec3 diff = vec3(sampleDiffuse(matId, p) * light.Id * max(0, dot(l, n)));
    vec3 shine = vec3(ks * light.Is * pow(max(0, dot(v, r)), shininess));
    
    color = vec3(amb + diff + shine);
...
vec3 sampleDiffuse(int matId, vec3 p) 
{
    ...
    if(matId == 0) {		
        ...
        vec4 texture_color = texture(floor_color, uv);
        color = mat_color * texture_color.rgb;
...
bool isShadowed(Light light, Hit h) 
{
    ...
    Ray shadowRay;
    shadowRay.ori = intersect + (toLight) * Epsilon; // note to self: shadow starts at where the light hits & offset with epsilon
    shadowRay.dir = dir;

    Hit testing = findHit(shadowRay);

    if (testing.t > 0 && testing.t < t_max) {
        shadowed = true;
    }
...
float softShadow(Light light, Hit hit) {
    int SAMPLES = 8;
    float shadow = 0.0;
    ...
    for (int i = 0; i < SAMPLES; i++) {
        ...
        // random jitter around the light
        vec2 r = rand2(g_seed) * 2.0 - 1.0;
        float lightRadius = 0.5;

        vec3 jitter = vec3(r.x, 0.0, r.y) * lightRadius;
        vec3 randomLightPos = light.position + jitter;

        vec3 toLight = randomLightPos - intersect;
        float t_max = length(toLight);
        vec3 dir = normalize(toLight);

        Ray shadowRay;
        shadowRay.ori = intersect + toLight * Epsilon;
        shadowRay.dir = dir;

        Hit testing = findHit(shadowRay);

        if (testing.t > 0.0 && testing.t < t_max)
            shadow += 1.0;
    }

    return shadow / float(SAMPLES); // return 0–1 percentage
}
...
vec3 rayTrace(in Ray r, out Hit hit) 
{
    ...
    if(h.t > 0 && h.t < 1e8) {
        ...
        for(int i = 0; i < lights.length(); i++) {
            float sh = softShadow(lights[i], h);
            ...
            if (sh > 0.0) {
                col += mix(shadingPhong(lights[i], h.matId, e, p, s, n), materials[h.matId].ka * lights[i].Ia, sh);
            } else {
...
void main() 
{
    ...
    for(int i = 0; i < recursiveDepth; i++) {
        ...
        recursiveRay.ori = intersect + (reflected_dir * Epsilon);
